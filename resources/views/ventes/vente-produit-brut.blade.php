...
<div id="displaying_erea" class="group-data-[sidebar-size=lg]:ltr:md:ml-vertical-menu group-data-[sidebar-size=lg]:rtl:md:mr-vertical-menu group-data-[sidebar-size=md]:ltr:ml-vertical-menu-md group-data-[sidebar-size=md]:rtl:mr-vertical-menu-md group-data-[sidebar-size=sm]:ltr:ml-vertical-menu-sm group-data-[sidebar-size=sm]:rtl:mr-vertical-menu-sm pt-[calc(theme('spacing.header')_*_1)] pb-[calc(theme('spacing.header')_*_0.8)] px-4 group-data-[navbar=bordered]:pt-[calc(theme('spacing.header')_*_1.3)] group-data-[navbar=hidden]:pt-0 group-data-[layout=horizontal]:mx-auto group-data-[layout=horizontal]:max-w-screen-2xl group-data-[layout=horizontal]:px-0 group-data-[layout=horizontal]:group-data-[sidebar-size=lg]:ltr:md:ml-auto group-data-[layout=horizontal]:group-data-[sidebar-size=lg]:rtl:md:mr-auto group-data-[layout=horizontal]:md:pt-[calc(theme('spacing.header')_*_1.6)] group-data-[layout=horizontal]:px-3 group-data-[layout=horizontal]:group-data-[navbar=hidden]:pt-[calc(theme('spacing.header')_*_0.9)]">
                                ...
                                <form id="formulaire_ajout">
                                    ...
                                    <div class="flex mb-2">
                                        <div class="col mr-2 w-full">
                                            ...
                                            <select required id="type_de_vente_add" name="type_de_vente" class="ltr:pl-8 rtl:pr-8 form-select border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200 mr-2 w-full">
                                    ...
                                    {{-- Rappelons que chaque vente contient en elle plusieurs produits --}}
                                        ...
                                        <div class="flex mb-2">
                                            <div class="col mr-2 w-full">
                                                ...
                                                <select multiple name="produits[0]" class="element_a_vendre_selecte ltr:pl-8 rtl:pr-8 form-select border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200 mr-2 w-full">
                                                    <option value="-1" disabled selected hidden>Choisissir un produit...</option>
                                    ...
                                    <form id="formulaire_modif">
                                        ...
                                        <div class="flex mb-2">
                                            <div class="col mr-2 w-full">
                                                ...
                                                <input readonly type="text" name="reference" id="reference_modif" class="row ltr:pl-8 rtl:pr-8 form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200 w-full" placeholder="Tapez ici..." autocomplete="on">
                                            </div>
                                        </div>
    ...
    <script>
        ...
        function addInputEventToSpecificInput(element){
            ...
            const prix_vente = $(element + ' .prix_vente');
            ...
            $( element + ' .montant_regle').on('input', function(){
                if($('#type_de_vente_add').val() == "Au comptant"){
                    if($(this).val() < (quantite_vendue.val() * prix_vente.val())){
                        $(this).css("border", "1px solid rgba(255, 0, 0, 0.5)");
                    }else if($(this).val() == (quantite_vendue.val() * prix_vente.val())){
            ...
            element_a_vendre_selecte.on('change', function(){
                const ligne_client = $('#ligne_client_systeme_selecte').val();
                $.ajax({
                    url: "{{ route('render-product_properties_for_vente') }}",
                    method: 'GET',
                    data: { 
                        id_produit: $(this).val()[0],
                        ligne_client : ligne_client[0],
                    },
                    success: function(response) {
                        if(response){
                            if(response.produit_brut){
                                $(element + ' .element_a_vendre_input').val(response.produit_brut.produit.designation);
                                $(element + ' .prix_vente').val(response.produit_brut.puv);
            ...
            $.ajax({
                url: "{{route('render_produits_avec_type_vente_pb')}}",
                method: 'GET',
                data: { 
                    typeVente: document.getElementById('type_de_vente_add').value,
                    client: document.getElementById('ligne_client_systeme_selecte').value,
        ...
        function ajouter(){
            ...
            updateClients();
            clients_on_change("{{route('render_produits_avec_type_vente_pb')}}");
        }
        ...
        function updateClients(){    
            ...
            ligne_client_systeme_input.addEventListener('input', function(e) {
                ...
                timeId = setTimeout(function() {
                    ...
                    let typeVente = document.getElementById('type_de_vente_add').value;
                    $.ajax({
                        url: '/rechercher-clients-pour-vente',
                        method: 'GET',
                        data: { terme: inputValue, typeVente : typeVente},
                        success: function(response) {
                ...
                success: function(response) {
                    ...
                    $.each(lignesVentes, function(index, lignesVente) {
                        if (lignesVente.systeme_produit) {
                            ...
                            let valeur_invendue = lignesVente.prix_vente * invendue;
                            let totale_valeur = lignesVente.prix_vente * lignesVente.quantite_envoyee;
                            let reste_a_regler = valeur_vendue - lignesVente.montant_regle;

                            let row = '<tr data-id="' + lignesVente.id + '">' +
               ...
               request.onreadystatechange = function() {
                ...
                if (request.readyState === XMLHttpRequest.DONE) {
                    if (request.status === 200) {
                        ...
                        if(response.quantiteInsufisante != undefined){toastr.warning(response.quantiteInsufisante); return;}
                        let errorMessageElements = document.querySelectorAll('.error-message');
                        if(errorMessageElements.length > 0){
                            errorMessageElements.forEach(function(element) {
                                element.parentNode.removeChild(element);
        ...
        selectRenderClientsForVentes('render_clients_avec_type_vente_pb');
    </script>   
@endsection